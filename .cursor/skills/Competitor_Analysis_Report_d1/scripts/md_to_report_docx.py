...
def _set_run_fonts(run, font_zh: str, font_en: str | None = None, size_pt: float | None = None, bold: bool = False):
...
def _add_paragraph_with_inline(doc, text: str, as_bullet: bool = False):
    ...
    p.paragraph_format.space_after = Pt(0)
    p.paragraph_format.line_spacing = LINE_SPACING_BODY
    for segment, bold in _parse_inline_bold(text):
        if not segment:
            continue
        r = p.add_run(segment)
        _set_run_fonts(r, FONT_BODY_ZH, FONT_BODY_EN, PT_BODY, bold=bold)
    # 不再调用 _style_body(p)，避免覆盖 ** 解析出的粗体


def add_para(doc, text, as_bullet: bool = False):
    """添加正文段落；支持 **粗体**、去除首行 '- '、忽略 '---'。"""
    text = (text or "").strip()
...
def parse_md(md_path: Path) -> list:
    """解析 report_draft.md：返回 [(type, content), ...]，type 为 'h1'|'h2'|'h3'|'para'|'list'。"""
    ...
    for seg in segments:
        ...
        if first.startswith("## ") and not first.startswith("### ") and not first.startswith("#### "):
            blocks.append(("h1", first[3:].strip()))
            rest = " ".join(lines[1:])
            ...
            blocks.append(("h3", first[5:].strip()))
            rest = " ".join(lines[1:])
            if rest and rest not in ("---", "***", "----"):
                blocks.append(("para", rest))
        elif first.startswith("# ") and not first.startswith("## "):
