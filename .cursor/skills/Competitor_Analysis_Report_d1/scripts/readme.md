...
- 所有脚本请在**项目根目录**下执行。
- 使用前请先创建并激活项目虚拟环境，再安装依赖：
  - **创建虚拟环境**（若尚未创建）：`python -m venv .venv`
  - **激活虚拟环境**：
    - Windows (PowerShell)：`.venv\Scripts\Activate.ps1`
    - Linux / macOS (bash)：`source .venv/bin/activate`
  - **安装依赖**：在已激活虚拟环境的终端中执行  
    `pip install -r .cursor/skills/Competitor_Analysis_Report_d1/requirements.txt`
- 未激活虚拟环境时，`run_markitdown` 等可能因依赖或 Python 版本冲突而失败。
- 以下示例命令在 Linux/macOS (bash) 或 Windows (PowerShell) 中均可执行。
