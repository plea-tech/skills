...
-子目录下有一个docs_input.md，用来说明每个文档名字和内容. 文档的语言包括中文和英文.
-子目录下有一个可选的links_input.md，用来说明竞品分析用到的一些网页链接，你需要读取这些链接的信息.
...
2.1 对于文字内容, 按你自己的理解和分析生成markdown格式的文档.
2.2 对于图片: 
...
# 脚本说明
## 路径约定
项目目录指当前工作区根目录。extracted_content、reports 均在其下。竞品参考资料在技能目录的 references 下，路径为：.cursor/skills/Competitor_Analysis_Report_d1/references/<竞品名>。若用户只给出竞品名或别名，可先运行 resolve_competitor 得到竞品目录路径。Step 3 迭代1 的 **markdown 草稿** 保存为 extracted_content/<竞品名>/report_draft.md，再据此整理为 reports/ 下的 docx。
## 调用约定
...
- 脚本路径为 .cursor/skills/Competitor_Analysis_Report_d1/scripts/<脚本名>.py。
